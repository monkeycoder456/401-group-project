...
var aura_on_cooldown := false
@export var aura_cooldown_time := 20.0
...
func spawn_aura():
	#Spawns the aura onto the character
	if current_aura != null or aura_on_cooldown:
		return

	if aura_scene != null:
		current_aura = aura_scene.instantiate()
		add_child(current_aura)
		current_aura.tree_exited.connect(_on_aura_removed)


func _on_aura_removed():
	#Removes the aura and begins the cooldown
	current_aura = null
	start_aura_cooldown()
	
func start_aura_cooldown():
	#Cooldoown for the function begins
	#after 20 seconds the cooldown ends
	aura_on_cooldown = true
	print("Aura on cooldown...")

	await get_tree().create_timer(aura_cooldown_time).timeout

	aura_on_cooldown = false
	print("Aura ready!")
