...
function drawLine_Reletive_to_point(x,y,direction,xScale,yScale){
    ctx.beginPath();
    ...
    //console.log(sumY_second_point)
    // ctx.moveTo(x,y);
    ctx.moveTo(x + xScale*sumX_first_point,y + yScale*sumY_first_point);
    ctx.lineTo(x + xScale*sumX_second_point,y + yScale*sumY_second_point);
    ctx.stroke();
...
function drawMaze_Debug(maze,startx,starty,scalex,scaley){
    /*
    startx is the beginning x position
    starty is the beginning y position
    scalex is the size of the maze along the x line
    scaley is the size of the maze along the y line
    psudo code
   ...
   const pathcolor = ["green","Aquamarine"]
   let x_pos = startx;
   let y_pos = starty;
   let iterate = 0;
    for (let i = 0; i < maze.maze.length; i++){//this would give you the row
        // //console.log(i)
        for (let j = 0; j < maze.maze[i].length; j++){//this would give you the column
            // //console.log(j);
            const direcrtions = Object.keys(maze.maze[i][j])
            direcrtions.forEach(d => {
                //console.log(maze.maze[i][j][d])
                if(maze.maze[i][j][d] == true){
                    // //color a green triangle
                    // ctx.stokeStyle = pathcolor[iterate % 2]
                    // drawLine_Reletive_to_point(x_pos,y_pos,d,scalex,scaley)
                    // //console.log("green")

                }else{
                    //color a black triangle
                    ctx.stokeStyle = color[iterate % 2]
                    drawLine_Reletive_to_point(x_pos,y_pos,d,scalex,scaley)
                    //console.log("black")
                }

            });
            //we finished drawing one square. move right 1
            iterate += 1;
            x_pos += scalex * 2;

        }
        //iterated an entire row
        //move one down, reset xpos
        x_pos = startx
        y_pos += scaley * 2
    }
...
class Maze{
    constructor(rows,columns){
        ...
        for (let i = 0; i < this.maze.length; i++){//this would give you the row
            ...
            for (let j = 0; j < this.maze[i].length; j++){//this would give you the column
                ...
                this.maze[i][j] = {"N":false,"S":false,"E":false,"W":false};
            }
        }
    }
    ...
    getCarvedStatus(row,column){
        ...
        if(path_in_question["N"] == true ||path_in_question["S"] == true||path_in_question["W"] == true||path_in_question["E"] == true){
            return true;
        }
        return false;
    }
    ...
    */
    ...
    if (direction == "N"){
        this.maze[coordPair.y][coordPair.x]["N"] = true;
        this.maze[coordPair.y-1][coordPair.x]["S"] = true;
    }
    if (direction == "S"){
        this.maze[coordPair.y][coordPair.x]["S"] = true;
        this.maze[coordPair.y+1][coordPair.x]["N"] = true;
    }
    if (direction == "E"){
        this.maze[coordPair.y][coordPair.x]["E"] = true;
        this.maze[coordPair.y][coordPair.x+1]["W"] = true;
    }
    if (direction == "W"){
        this.maze[coordPair.y][coordPair.x]["W"] = true;
        this.maze[coordPair.y][coordPair.x-1]["E"] = true;
    }
